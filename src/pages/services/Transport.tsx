...
const Transport = () => {
  ...
  // Fetch transport services from database
  const { services: transportServices, isLoading, error } = useServices({ serviceType: 'transport' });

  // Transform database services to display format
  const services = useMemo(() => {
    if (transportServices.length > 0) {
      return transportServices.map(service => ({
        icon: service.category === 'airport' ? Plane : service.category === 'driver' ? UserCheck : Car,
        title: service.title,
                  ...
                  <div className="relative h-48 overflow-hidden">
                    {service.image ? (
                      ...
                      <div className="w-full h-full bg-muted flex items-center justify-center">
                        <service.icon className="w-16 h-16 text-muted-foreground" />
                      </div>
                    )}
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              <div className="mt-8 p-6 bg-primary/5 rounded-2xl">
                <h4 className="font-bold text-primary mb-4">{t('transportPage.includedTitle')}</h4>
                <ul className="space-y-3">
                  {[
                    t('transportPage.included1'),
                    t('transportPage.included2'),
                    t('transportPage.included3'),
                    t('transportPage.included4')
                  ].map((feature) => (
