/**
 * Checkout page - Guest checkout with email, shipping, and order confirmation
 * No authentication required. No online payment — orders are confirmed via contact.
 */
...
import { z } from "zod";
import { 
  ShoppingBag, CheckCircle, Truck, MapPin, User, Mail, Phone, 
  FileText, ChevronRight, Shield, Package, ArrowRight, Globe
} from "lucide-react";
...
// Validation schema factory - no payment fields needed
...
const Checkout = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Get current user session (optional - guest checkout allowed)
      ...
      const orderItems = cartItems.map(item => ({
        ...
        unit_price: item.product?.promoPrice || item.product?.price || 0,
      }));
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="bg-secondary/10 rounded-2xl border border-secondary/20 p-6 text-center">
                  <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <h2 className="font-title font-semibold text-foreground">{t('checkout.paymentOnDelivery', 'Paiement à la livraison')}</h2>
                  </div>
                  <p className="text-sm text-muted-foreground font-base">
                    {t('checkout.paymentOnDeliveryDesc', 'Vous serez contacté(e) par notre équipe pour confirmer votre commande et organiser le paiement.')}
                  </p>
                </div>
                ...
                >
                  {loading ? (
                    ...
                    <>
                      ...
                      {t('checkout.confirmOrder', 'Confirmer la commande')} — {total.toLocaleString()}€
                    </>
                  )}
                </Button>
