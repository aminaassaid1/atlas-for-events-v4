...
// Fallback images for testimonials without author_image
...
const TestimonialsSection = () => {
    ...
    const fetchTestimonials = async () => {
      ...
      if (!error && data && data.length > 0) {
        setTestimonials(data.map((t, i) => ({
          name: t.author_name,
          role: t.author_location || "",
          image: t.author_image || fallbackImages[i % fallbackImages.length],
          rating: t.rating,
          text: t.content,
        })));
      }
  ...
  return <section className="py-16 lg:py-24 relative overflow-hidden bg-background">
      <div className="container mx-auto px-4">
        {/* Title */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center max-w-2xl mx-auto mb-6 lg:mb-10">
        ...
        <div className="relative mb-0 lg:mb-4 overflow-hidden">
          ...
          <motion.img src={airplaneTakeoff} alt="Airplane" className="absolute left-1/2 -translate-x-1/2 -top-1 sm:-top-2 md:-top-4 lg:-top-6 xl:-top-8 w-[50%] sm:w-[45%] md:w-[40%] lg:w-[30%] max-w-md z-10" animate={{ y: [0, -6, 0] }} transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }} />
        </div>
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 lg:gap-8">
            ...
            <div className="relative flex-shrink-0 z-10">
              ...
              <AnimatePresence mode="wait">
                <motion.div key={currentIndex} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} transition={{ duration: 0.4 }} className="relative z-10">
            ...
            <div className="xl:absolute xl:left-[520px] xl:top-1/2 xl:-translate-y-1/2 flex xl:flex-col gap-2 lg:gap-3 order-3 md:order-2 mt-10 md:mt-0 justify-center">
              {testimonials.map((testimonial, index) => <motion.button key={index} onClick={() => setCurrentIndex(index)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className={`relative transition-all duration-300 rounded-xl overflow-hidden ${index === currentIndex ? 'ring-3 ring-atlas-gold ring-offset-2 scale-105' : 'opacity-50 hover:opacity-100'}`}>
              ...
              <AnimatePresence mode="wait">
                <motion.div key={currentIndex} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.4 }}>
                  <div className="flex items-start justify-between mb-3 sm:mb-4 lg:mb-6 max-sm:flex-col max-sm:text-center max-sm:gap-2">
                    ...
                    <Quote className="w-10 h-10 sm:w-12 sm:h-12 lg:w-16 lg:h-16 text-primary max-sm:mx-auto flex-shrink-0" style={{ transform: 'scaleX(-1)' }} />
                  </div>
